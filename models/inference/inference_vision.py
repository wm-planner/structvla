...
import math
import time
# Keep this near the existing imports
from peft import PeftModel
sys.path.append("/remote-home/jinminghao/structvla/reference/Emu3")
from emu3.mllm import Emu3Planner  # The same class used in LoRA training
...
def load_vllm_model(model_path):
    ...
    try:
        ...
        raise e
def load_torch_model_with_lora(base_model_dir, adapter_dir, merge_lora=False):
    """
    Correct LoRA loading flow: load the base Emu3Planner first, then apply the
    LoRA adapter (optionally merged).
    """
...
def parse_args():
    ...
    parser.add_argument("--predict_frames", type=int, default=5, help="Number of frames to predict") #5
    ...
    parser.add_argument("--output_dir", type=str, default="/remote-home/jinminghao/structvla/logs/planner_libero_action_vision_with_gripper_10k_1205", 
                        help="Output directory for visualizations")
    parser.add_argument("--use_lora", action="store_true",
                        help="Enable LoRA adapter at inference.")
    parser.add_argument("--base_model_dir", type=str, default=None,
                        help="Base model directory (the same as --model_name_or_path used in training).")
    parser.add_argument("--adapter_dir", type=str, default=None,
                        help="LoRA adapter checkpoint folder (the checkpoint-NNNN with adapter files).")
    parser.add_argument("--merge_lora", action="store_true",
                        help="Merge LoRA into base weights for faster inference (no PEFT at runtime).")
...
if __name__ == "__main__":
    ...
    train_pickle = '/remote-home/jinminghao/structvla/datasets/processed_data/meta/libero_all_norm.pkl'
    with open(train_pickle, 'rb') as f:
        train_meta = pickle.load(f)

    # LoRA-related arguments
    BASE_MODEL_DIR = args.base_model_dir  # Newly added
    ADAPTER_DIR    = args.adapter_dir     # Newly added

    EMU_HUB = "/remote-home/jinminghao/structvla/ckpt/world_model_libero_10k"
    VQ_HUB = "/remote-home/jinminghao/structvla/pretrain/Emu3-Base"
    VISION_HUB = "/remote-home/jinminghao/structvla/pretrain/Emu3-VisionVQ"
    #VQ_HUB = "/remote-home/jinminghao/structvla/pretrain/Emu3-Stage1"
    #VISION_HUB = "/remote-home/jinminghao/structvla/pretrain/Emu3-VisionTokenizer"
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    
    # Load model based on selected inference mode
    temp_dir_to_cleanup = None
   
    # vLLM + LoRA is not recommended here; vLLM does not directly load PEFT adapters
    if args.use_lora and args.use_vllm:
        print("[WARN] vLLM and LoRA(PEFT) are unstable when used together directly and have been reverted to PyTorch inference.")
        use_vllm = False

    # Choose the loading path based on the flags
    temp_dir_to_cleanup = None
    if args.use_lora:
        assert BASE_MODEL_DIR is not None and ADAPTER_DIR is not None, \
            "--base_model_dir and --adapter_dir all needed to be set when using LoRA."
        print("Using PyTorch + LoRA adapter for inference")
        model = load_torch_model_with_lora(
    ...
    # Loading from VQ_HUB directly can ignore vision token files saved into the checkpoint during training
    # When LoRA is enabled, prefer adapter_dir first, then base_model_dir; otherwise keep using the original VQ_HUB
    if args.use_lora:
        tok_try = ADAPTER_DIR if ADAPTER_DIR is not None else BASE_MODEL_DIR
        tok_fallback = BASE_MODEL_DIR
    else:
        tok_try = VQ_HUB
        tok_fallback = VQ_HUB
    ...
    task_indices = list(range(min(video_average_nums, len(train_meta))))
    print(f"Total available tasks: {len(train_meta)}")
    print(f"Total tasks to be sampled: {video_average_nums}")
    while saved_count < video_average_nums:
        ...
        rand_idx = 5 # For debugging, always use the first sequence 
        ...
        output_name = os.path.join(OUTPUT_DIR, f"video_{saved_count}_{task_idx}_{rand_idx}.png")
