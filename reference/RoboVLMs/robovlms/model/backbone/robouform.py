...
class RoboUform(BaseRoboVLM):
    ...
    def hidden_size(self):
        return self.backbone.text_decoder.config.hidden_size

    @property
    def word_embedding(self):
        return self.backbone.text_decoder.get_input_embeddings()

    @property
    def text_tower(self):
        return self.backbone.text_decoder

    @property
    def vision_tower(self):
        return self.backbone.image_encoder
    ...
    def model_encode_images(self, images):
        return self.backbone.get_images_embeddings(images)
...
if __name__ == "__main__":
    configs = load_config(
        "configs/finetune_uform_cont-lstm-post_full-ft_text_vision_wd=0_hist=8_act=10_aug-shift_act-norm_lr-2e-5.json"
    )
