...
class Emu3Processor(ProcessorMixin):
    ...
    def multimodal_decode_video(self, doc):
        ...
        for c in chunks:
            ...
            if self.tokenizer.boi_token in c:
                video = []
                frame_chunks = re.split(re.escape(self.tokenizer.eof_token), c)  # Split by frame
                for frame in frame_chunks:
                    token_ids = re.findall(self.visual_template[1], frame)  # Extract all visual tokens
                    if len(token_ids) > 0:
                        image = torch.tensor([int(m) for m in token_ids], dtype=torch.long,
                                            device=self.vision_tokenizer.device)  # Build tensor
    ...
    def add_prefix_template(self, frames, h, w):
        video_prompt = (
            ...
            self.tokenizer.eoi_token + 
            self.tokenizer.boi_token +
            f"{frames}*{h}*{w}" +  # Number of frames, height, and width of the video
            self.tokenizer.img_token  # Video start marker
        )
    ...
    def format_video_prompt(self, video_tokens, context_frames=0):
        # Assume video_tokens is a tensor of shape [frames, height, width]
