...
class Emu3PrefixConstrainedVideoLogitsHelper:
    ...
    def __call__(self, batch_id, input_ids):
        ...
        # Compute frame index and within-frame offset
        # frame_offset = offset // (width * height+1)
        # in_frame_offset = offset % (width * height+1)

        # if frame_offset == frames:  # All frames processed, return EOI
        #     return (self.eoi_token,)
        # elif in_frame_offset == 0:  # Current frame finished, return EOF
        #     return (self.eof_token,)
        # elif offset == (width * height + 1) * frames + 1:  # Sequence finished, return EOS
        #     return (self.eos_token,)
        # elif offset > (width * height + 1) * frames + 1:  # Out of range, return PAD
        #     return (self.pad_token,)
        # else:  # Return a visual token
        #     return self.visual_tokens
