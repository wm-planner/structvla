...
class Emu3RotaryEmbedding(nn.Module):
    ...
    def _set_cos_sin_cache(self, seq_len, device, dtype):
        ...
        # Force float32 and avoid relying on arange(dtype=...) to reduce environment-dependent behavior
...
class Emu3SdpaAttention(Emu3Attention):
    ...
    def forward(
        ...
        # Convert 4D [B,1|H,Q,K] to the SDPA-compatible 3D [B*H,Q,K] format (additive bias or bool)
        # === Accept 4D additive/bool mask: [B, 1|H, Q, K]; don't reshape to [B*H,Q,K] ===
        if attention_mask is not None:
            am = attention_mask
            if am.dim() != 4 or am.size(0) != bsz or am.size(2) != q_len or am.size(3) != kv_seq_len:
                raise ValueError(
                    f"attn_mask must be 4D [B, 1|H, Q, K] with Q={q_len},K={kv_seq_len}, got {tuple(am.shape)}"
                )
            # Normalize to [B, H, Q, K]; if the second dimension is 1, broadcast across heads
            if am.size(1) == 1:
                am = am.expand(bsz, self.num_heads, q_len, kv_seq_len)
            elif am.size(1) != self.num_heads:
                raise ValueError(
                    f"attn_mask second dim must be 1 or num_heads={self.num_heads}, got {am.size(1)}"
                )
            # Allow bool masks (True=masked) or float masks (additive bias, 0=allow, large negative=mask)
            if am.dtype not in (torch.bool, torch.float16, torch.bfloat16, torch.float32):
                am = am.to(dtype=torch.float32)
        ...
        if attention_mask is not None:
            ...
            # Broadcast [B,1,Q,K] to [B,H,Q,K] across heads
            if am.size(1) == 1:
                am = am.expand(bsz, self.num_heads, q_len, kv_seq_len)
            elif am.size(1) != self.num_heads:
                raise ValueError(f"attn_mask second dim must be 1 or num_heads={self.num_heads}, got {am.size(1)}")

            # Key point: cast float masks to the same dtype as query; keep bool masks unchanged
            if am.dtype is not torch.bool:
                am = am.to(dtype=q_dtype)

            attention_mask = am.contiguous()

        # Keep Q/K/V contiguous when using custom masks on CUDA
        if query_states.device.type == "cuda" and attention_mask is not None:
            query_states = query_states.contiguous()
            key_states = key_states.contiguous()
            value_states = value_states.contiguous()
        ...
        attn_output = torch.nn.functional.scaled_dot_product_attention(
            ...
            key_states,
            value_states,
            attn_mask=attention_mask,   # Now [B,H,Q,K], with dtype matching query
            dropout_p=self.attention_dropout if self.training else 0.0,
            is_causal=False,            # A full block mask is already provided
        )
...
class CrossAttnBlock(nn.Module):
    ...
    def forward(self, x, mem):
        # x: [B, Lx, D]  (action tokens)
        # mem: [B, Lm, D] (condition tokens)
        q = self.ln_q(x)
        kv = self.ln_kv(mem)
...
class Emu3Planner(Emu3PreTrainedModel):
    _tied_weights_keys = ["lm_head.weight"]
    ...
    def forward(
        ...
        return_dict: Optional[bool] = None,
        action: Optional[torch.Tensor] = None,
        loss_weight: Optional[torch.Tensor] = None,   # Newly added
    ) -> Union[Tuple, CausalLMOutputWithPast]:
        r"""
        Args:
            labels (`torch.LongTensor` of shape `(batch_size, sequence_length)`, *optional*):
        ...
        if labels is not None:
            ...
            # Optional positional weights; use all ones if the dataset does not provide them
            if loss_weight is not None:
                shift_weight = loss_weight[..., 1:].to(shift_logits.dtype)  # [B, S-1]
            else:
                shift_weight = torch.ones_like(shift_labels, dtype=shift_logits.dtype)

            # Class weights (the original visual class weighting)
            class_w = None
            if self.use_weight:
                class_w = torch.ones(self.config.vocab_size, device=shift_logits.device)
                vr = range(self.bov_token_id, self.eov_token_id + 1)
                class_w[vr] = self.vision_loss_weight

            # Compute CE per token, then apply position-wise weighted averaging
            ...
            valid = (shift_labels != -100).to(per_tok.dtype)      # Ignore -100
            w = shift_weight * valid
            denom = w.sum().clamp_min(1.0)
            loss_ce = (per_tok * w).sum() / denom
